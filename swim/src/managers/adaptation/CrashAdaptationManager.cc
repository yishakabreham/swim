...
#include "CrashAdaptationManager.h"
#include "managers/adaptation/UtilityScorer.h"
#include "managers/execution/AllTactics.h"

using namespace std;

Define_Module(CrashAdaptationManager);
 ...
 * - if isServerCrashing
 *      - if total active servers = 1, then add new server else do nothing, then remove the crashed server
 * else, proceed with
 ...
 */
Tactic* CrashAdaptationManager::evaluate() {
    ...
    double crashChance = getSimulation() -> getSystemModule() -> par("crashChance");
    bool isServerCrashing = random < crashChance;

    if (isServerCrashing) {
        cout << "-- Server crashing -- \n";
        if (pModel -> getServers() == 1) {
            cout << "Total server before crashing = 1\n";
            pMacroTactic -> addTactic(new AddServerTactic);
        }
        pMacroTactic -> addTactic(new RemoveServerTactic);
    } else {
        if (responseTime > RT_THRESHOLD) {
            cout << "-- The goal is violated. -- \n";
            if (!isServerBooting
